...
(async () => {
  ...
  const latestActivityDetailHref = await page.getAttribute("#openLatestActivityLink", "href");
  ...
  const latestActivityDetailMode = await page.getAttribute("#openLatestActivityLink", "data-action-mode");
  if (latestActivityDetailMode !== "open_report") throw new Error("backend-owned latest activity action mode missing from dataset detail: " + latestActivityDetailMode);
	...
	const recentRunOpenRunHref = await page.getAttribute('a[data-recent-run-open-run="' + runID + '"]', "href");
	if (!recentRunOpenRunHref || !recentRunOpenRunHref.includes("/admin/eval-runs?") || !recentRunOpenRunHref.includes("run_id=" + encodeURIComponent(runID))) throw new Error("recent activity open-run handoff missing canonical run target");
	const recentRunOpenRunMode = await page.getAttribute('a[data-recent-run-open-run="' + runID + '"]', "data-action-mode");
	if (recentRunOpenRunMode !== "open_run") throw new Error("recent activity open-run action mode missing canonical run state: " + recentRunOpenRunMode);
	const recentRunOpenReportHref = await page.getAttribute('a[data-recent-run-open-report="' + runID + '"]', "href");
	if (!recentRunOpenReportHref || !recentRunOpenReportHref.includes("/admin/eval-reports?") || !recentRunOpenReportHref.includes("selected_report_id=" + encodeURIComponent(reportID))) throw new Error("recent activity open-report handoff missing canonical report target");
	const recentRunOpenReportMode = await page.getAttribute('a[data-recent-run-open-report="' + runID + '"]', "data-action-mode");
	if (recentRunOpenReportMode !== "open_report") throw new Error("recent activity open-report action mode missing canonical report state: " + recentRunOpenReportMode);
	const preferredQueueHref = await page.getAttribute("#openPreferredQueueLink", "href");
	if (!preferredQueueHref) throw new Error("recent activity preferred queue handoff missing from dataset detail");
	const preferredQueueMode = await page.getAttribute("#openPreferredQueueLink", "data-action-mode");
	if (preferredQueueMode !== "open_latest_report_queue") throw new Error("preferred queue action mode missing from dataset detail: " + preferredQueueMode);
  const datasetCaseHref = await page.getAttribute("#openDatasetCaseQueueLink", "href");
  if (!datasetCaseHref) throw new Error("linked dataset case handoff missing from dataset detail");
  const datasetCaseMode = await page.getAttribute("#openDatasetCaseQueueLink", "data-action-mode");
  if (datasetCaseMode !== "open_existing_case") throw new Error("linked dataset case action mode missing from dataset detail: " + datasetCaseMode);
  const runBackedCaseHref = await page.getAttribute("#openRunBackedCaseLink", "href");
  if (!runBackedCaseHref) throw new Error("run-backed case handoff missing from dataset detail");
  const runBackedCaseMode = await page.getAttribute("#openRunBackedCaseLink", "data-action-mode");
  if (runBackedCaseMode !== "open_existing_case") throw new Error("run-backed case action mode missing from dataset detail: " + runBackedCaseMode);
...
func TestAdminEvalReportsPageRendersHTML(t *testing.T) {
	...
	if !strings.Contains(body, "data-bad-case-follow-up-lane-action") {
		t.Fatal("bad-case follow-up lane selector missing from eval reports page HTML")
	}
...
func TestAdminEvalReportComparePageRendersHTML(t *testing.T) {
	...
	if !strings.Contains(body, "Open left eval report lane") {
		t.Fatal("left eval report lane handoff missing from compare HTML")
	}
	...
	if !strings.Contains(body, "Open right eval report lane") {
		t.Fatal("right eval report lane handoff missing from compare HTML")
	}
...
(async () => {
  ...
  const leftReportLaneHref = await page.getAttribute("#leftReportLaneLink", "href");
  if (!leftReportLaneHref || !leftReportLaneHref.includes("/admin/eval-reports?") || !leftReportLaneHref.includes("selected_report_id=" + encodeURIComponent(leftReportID))) {
    throw new Error("left report lane handoff missing canonical eval-report lane target");
  }
  const leftReportLaneMode = await page.getAttribute("#leftReportLaneLink", "data-action-mode");
  if (leftReportLaneMode !== "open_report") {
    throw new Error("left report lane handoff mode missing canonical open_report state: " + leftReportLaneMode);
  }
  const leftDatasetLaneHref = await page.getAttribute("#leftDatasetLaneLink", "href");
  if (!leftDatasetLaneHref || !leftDatasetLaneHref.includes("/admin/eval-datasets?") || !leftDatasetLaneHref.includes("selected_dataset_id=")) {
    throw new Error("left dataset lane handoff missing canonical eval-dataset lane target");
  }
  const leftDatasetLaneMode = await page.getAttribute("#leftDatasetLaneLink", "data-action-mode");
  ...
  const rightReportLaneHref = await page.getAttribute("#rightReportLaneLink", "href");
  if (!rightReportLaneHref || !rightReportLaneHref.includes("/admin/eval-reports?") || !rightReportLaneHref.includes("selected_report_id=" + encodeURIComponent(rightReportID))) {
    throw new Error("right report lane handoff missing canonical eval-report lane target");
  }
  const rightReportLaneMode = await page.getAttribute("#rightReportLaneLink", "data-action-mode");
  if (rightReportLaneMode !== "open_report") {
    throw new Error("right report lane handoff mode missing canonical open_report state: " + rightReportLaneMode);
  }
  const rightDatasetLaneHref = await page.getAttribute("#rightDatasetLaneLink", "href");
  if (!rightDatasetLaneHref || !rightDatasetLaneHref.includes("/admin/eval-datasets?") || !rightDatasetLaneHref.includes("selected_dataset_id=")) {
    throw new Error("right dataset lane handoff missing canonical eval-dataset lane target");
  }
...
func TestAdminEvalReportsPageRuntimeSmoke(t *testing.T) {
	...
	time.Sleep(2 * time.Millisecond)
	closedNoReuseBadCaseFollowUp, err := caseService.CreateCase(context.Background(), casesvc.CreateInput{
		TenantID:         "tenant-eval-admin-smoke",
		Title:            "Closed bad-case follow-up without report-level case",
		Summary:          "Closed bad-case follow-up summary without report-level case",
		SourceEvalCaseID: noReuseBadCaseEvalCaseID,
		CreatedBy:        "operator-eval",
	})
	if err != nil {
		t.Fatalf("CreateCase(closedNoReuseBadCaseFollowUp) error = %v", err)
	}
	if _, err := caseService.CloseCase(context.Background(), closedNoReuseBadCaseFollowUp.ID, "operator-eval"); err != nil {
		t.Fatalf("CloseCase(closedNoReuseBadCaseFollowUp) error = %v", err)
	}
...
(async () => {
  ...
  if (!badCaseFollowUpLaneHref || !badCaseFollowUpLaneHref.includes("/admin/cases?") || !badCaseFollowUpLaneHref.includes("case_id=" + encodeURIComponent(latestBadCaseFollowUpID))) {
    throw new Error("bad-case follow-up lane handoff missing canonical case target");
  }
  const badCaseFollowUpLaneMode = await page.locator('[data-bad-case-follow-up-lane-action="' + sourceEvalCaseID + '"]').first().getAttribute("data-action-mode");
  if (badCaseFollowUpLaneMode !== "open_existing_case") {
    throw new Error("bad-case follow-up lane handoff mode missing canonical open_existing_case state: " + badCaseFollowUpLaneMode);
  }
  ...
  const queueFollowUpLaneMode = await page.locator('[data-bad-case-follow-up-lane-action="' + noReuseBadCaseEvalCaseID + '"]').first().getAttribute("data-action-mode");
  if (queueFollowUpLaneMode !== "open_existing_queue") {
    throw new Error("bad-case follow-up lane queue handoff mode missing canonical open_existing_queue state: " + queueFollowUpLaneMode);
  }
  await page.click("#badCaseQuickViewNoFollowUp");
  await page.waitForFunction(() => {
    const params = new URL(window.location.href).searchParams;
