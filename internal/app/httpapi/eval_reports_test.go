...
func TestGetEvalReportReturnsMaterializedDetail(t *testing.T) {
	...
	if got.BadCases[0].PreferredFollowUpLaneAction.Mode != "create" {
		t.Fatalf("BadCases[0].PreferredFollowUpLaneAction.Mode = %q, want %q", got.BadCases[0].PreferredFollowUpLaneAction.Mode, "create")
	}
	if got.BadCases[0].PreferredFollowUpLaneAction.SourceEvalCaseID != got.BadCases[0].EvalCaseID {
		t.Fatalf("BadCases[0].PreferredFollowUpLaneAction.SourceEvalCaseID = %q, want %q", got.BadCases[0].PreferredFollowUpLaneAction.SourceEvalCaseID, got.BadCases[0].EvalCaseID)
	}
	...
	if got.BadCases[0].PreferredCaseSummaryAction.Mode != "none" {
		t.Fatalf("BadCases[0].PreferredCaseSummaryAction.Mode = %q, want %q", got.BadCases[0].PreferredCaseSummaryAction.Mode, "none")
	}
...
func TestGetEvalReportIncludesBadCaseFollowUpCaseSummary(t *testing.T) {
	...
	if badCase.PreferredFollowUpLaneAction.Mode != "open_existing_case" {
		t.Fatalf("BadCases[0].PreferredFollowUpLaneAction.Mode = %q, want %q", badCase.PreferredFollowUpLaneAction.Mode, "open_existing_case")
	}
	if badCase.PreferredFollowUpLaneAction.CaseID != openCase.ID {
		t.Fatalf("BadCases[0].PreferredFollowUpLaneAction.CaseID = %q, want %q", badCase.PreferredFollowUpLaneAction.CaseID, openCase.ID)
	}
	if badCase.PreferredFollowUpLaneAction.SourceEvalCaseID != evalCaseID {
		t.Fatalf("BadCases[0].PreferredFollowUpLaneAction.SourceEvalCaseID = %q, want %q", badCase.PreferredFollowUpLaneAction.SourceEvalCaseID, evalCaseID)
	}
	if badCase.PreferredCaseSummaryAction.Mode != "open_existing_case" {
		t.Fatalf("BadCases[0].PreferredCaseSummaryAction.Mode = %q, want %q", badCase.PreferredCaseSummaryAction.Mode, "open_existing_case")
	}
	if badCase.PreferredCaseSummaryAction.CaseID != openCase.ID {
		t.Fatalf("BadCases[0].PreferredCaseSummaryAction.CaseID = %q, want %q", badCase.PreferredCaseSummaryAction.CaseID, openCase.ID)
	}
	if badCase.PreferredCaseSummaryAction.SourceEvalCaseID != evalCaseID {
		t.Fatalf("BadCases[0].PreferredCaseSummaryAction.SourceEvalCaseID = %q, want %q", badCase.PreferredCaseSummaryAction.SourceEvalCaseID, evalCaseID)
	}
...
func TestGetEvalReportBadCaseLinkedCaseActionPrefersQueueWhenLatestCaseClosed(t *testing.T) {
	...
	if badCase.PreferredFollowUpLaneAction.Mode != "create" {
		t.Fatalf("BadCases[0].PreferredFollowUpLaneAction.Mode = %q, want %q", badCase.PreferredFollowUpLaneAction.Mode, "create")
	}
	if badCase.PreferredFollowUpLaneAction.CaseID != "" {
		t.Fatalf("BadCases[0].PreferredFollowUpLaneAction.CaseID = %q, want empty", badCase.PreferredFollowUpLaneAction.CaseID)
	}
	if badCase.PreferredFollowUpLaneAction.SourceEvalCaseID != evalCaseID {
		t.Fatalf("BadCases[0].PreferredFollowUpLaneAction.SourceEvalCaseID = %q, want %q", badCase.PreferredFollowUpLaneAction.SourceEvalCaseID, evalCaseID)
	}
	...
	if badCase.PreferredCaseSummaryAction.CaseID != closedCase.ID {
		t.Fatalf("BadCases[0].PreferredCaseSummaryAction.CaseID = %q, want %q", badCase.PreferredCaseSummaryAction.CaseID, closedCase.ID)
	}
	if badCase.PreferredCaseSummaryAction.SourceEvalCaseID != evalCaseID {
		t.Fatalf("BadCases[0].PreferredCaseSummaryAction.SourceEvalCaseID = %q, want %q", badCase.PreferredCaseSummaryAction.SourceEvalCaseID, evalCaseID)
	}
...
func TestCompareEvalReportsReturnsTypedSummary(t *testing.T) {
	...
	var got struct {
		Left struct {
			...
			PreferredDatasetLaneAction struct {
				Mode      string `json:"mode"`
				DatasetID string `json:"dataset_id"`
			...
			PreferredRunLaneAction struct {
				Mode      string `json:"mode"`
				RunID     string `json:"run_id"`
				DatasetID string `json:"dataset_id"`
			} `json:"preferred_run_lane_action"`
			PreferredTraceDetailAction struct {
				Mode     string `json:"mode"`
				ReportID string `json:"report_id"`
			} `json:"preferred_trace_detail_action"`
	...
	if got.Left.PreferredLinkedCaseAction.Mode != "open_existing_case" {
		t.Fatalf("Left.PreferredLinkedCaseAction.Mode = %q, want %q", got.Left.PreferredLinkedCaseAction.Mode, "open_existing_case")
	}
	if got.Left.PreferredLinkedCaseAction.CaseID != leftFollowUp.ID {
		t.Fatalf("Left.PreferredLinkedCaseAction.CaseID = %q, want %q", got.Left.PreferredLinkedCaseAction.CaseID, leftFollowUp.ID)
	}
	...
	if got.Left.PreferredTraceDetailAction.Mode != "open_trace" || got.Left.PreferredTraceDetailAction.ReportID != leftReportID {
		t.Fatalf("Left.PreferredTraceDetailAction = %#v, want open_trace/%q", got.Left.PreferredTraceDetailAction, leftReportID)
	}
	...
	if got.Right.PreferredLinkedCaseAction.Mode != "open_existing_case" {
		t.Fatalf("Right.PreferredLinkedCaseAction.Mode = %q, want %q", got.Right.PreferredLinkedCaseAction.Mode, "open_existing_case")
	}
	if got.Right.PreferredLinkedCaseAction.CaseID != rightFollowUp.ID {
		t.Fatalf("Right.PreferredLinkedCaseAction.CaseID = %q, want %q", got.Right.PreferredLinkedCaseAction.CaseID, rightFollowUp.ID)
	}
	...
	if got.Right.PreferredTraceDetailAction.Mode != "open_trace" || got.Right.PreferredTraceDetailAction.ReportID != rightReportID {
		t.Fatalf("Right.PreferredTraceDetailAction = %#v, want open_trace/%q", got.Right.PreferredTraceDetailAction, rightReportID)
	}
...
func TestCompareEvalReportsLinkedCaseActionPrefersQueueWhenLatestCaseClosed(t *testing.T) {
	caseService := casesvc.NewService()
	evalCaseService := evalsvc.NewService(caseService, nil)
	datasetService := evalsvc.NewDatasetService(evalCaseService)
	runService := evalsvc.NewRunService(datasetService)
	reportService := evalsvc.NewEvalReportServiceWithDependencies(nil, runService)
	leftReportID := materializeEvalRunReport(t, "tenant-eval-report-compare-linked-queue", evalsvc.RunStatusSucceeded, "success detail", caseService, evalCaseService, datasetService, runService, reportService, "Dataset Compare Queue A", "Source Left")
	rightReportID := materializeEvalRunReport(t, "tenant-eval-report-compare-linked-queue", evalsvc.RunStatusFailed, "failure detail", caseService, evalCaseService, datasetService, runService, reportService, "Dataset Compare Queue B", "Source Right")

	leftClosedCase, err := caseService.CreateCase(context.Background(), casesvc.CreateInput{
		TenantID:           "tenant-eval-report-compare-linked-queue",
		Title:              "Left closed linked case",
		Summary:            "left closed linked summary",
		SourceEvalReportID: leftReportID,
		CreatedBy:          "operator-left",
	})
	if err != nil {
		t.Fatalf("CreateCase(leftClosedCase) error = %v", err)
	}
	if _, err := caseService.CloseCase(context.Background(), leftClosedCase.ID, "operator-left"); err != nil {
		t.Fatalf("CloseCase(leftClosedCase) error = %v", err)
	}
	...
	resp, err := http.Get(server.URL + "/api/v1/eval-report-compare?tenant_id=tenant-eval-report-compare-linked-queue&left_report_id=" + leftReportID + "&right_report_id=" + rightReportID)
	if err != nil {
		t.Fatalf("Get() error = %v", err)
	}
	defer resp.Body.Close()
	...
	if got.Left.LinkedCaseSummary.TotalCaseCount != 1 || got.Left.LinkedCaseSummary.OpenCaseCount != 0 {
		t.Fatalf("Left linked case summary = %#v, want total=1 open=0", got.Left.LinkedCaseSummary)
	}
	if got.Left.PreferredLinkedCaseAction.Mode != "open_existing_queue" {
		t.Fatalf("Left.PreferredLinkedCaseAction.Mode = %q, want %q", got.Left.PreferredLinkedCaseAction.Mode, "open_existing_queue")
	}
	if got.Left.PreferredLinkedCaseAction.CaseID != "" {
		t.Fatalf("Left.PreferredLinkedCaseAction.CaseID = %q, want empty", got.Left.PreferredLinkedCaseAction.CaseID)
	}
	if got.Left.PreferredLinkedCaseAction.SourceEvalReportID != leftReportID {
		t.Fatalf("Left.PreferredLinkedCaseAction.SourceEvalReportID = %q, want %q", got.Left.PreferredLinkedCaseAction.SourceEvalReportID, leftReportID)
	}
