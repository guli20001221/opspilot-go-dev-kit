...
- that same eval-case list/detail contract now also includes typed `preferred_primary_action`, so `/admin/evals` can reuse linked-case history before it falls back to a new `Create case` write
- that same eval-case list/detail contract now also includes `preferred_linked_case_action`, so `/admin/evals` can open the latest linked case or the canonical eval-case queue from one backend-owned decision instead of branching on `latest_follow_up_case_id` in browser code
...
- those same canonical eval-dataset list/detail contracts now also expose `preferred_latest_activity_action`, so the dataset lane can use one backend-owned button for `Open latest run` versus `Open latest report`
...
- that same eval-report list/detail contract now also includes typed `preferred_primary_action`, so `/admin/eval-reports` can reuse linked-case history before it falls back to a new `Create case` write
...
- that same eval-report detail now also includes typed `preferred_primary_action` on each `bad_cases[]` row, so `/admin/eval-reports` bad-case actions follow one backend-owned create-versus-reuse decision instead of mixing follow-up and linked-case heuristics in page code
...
- that same compare contract now also includes typed `preferred_primary_action` per side, so `/admin/eval-report-compare` can reuse linked-case history before it falls back to compare queue or case creation
...
- consume `recent_runs[].preferred_primary_action` or the first button in each `Recent eval activity` row on `/admin/eval-datasets` when you want the canonical main handoff for one recent execution without recomputing run-vs-report-vs-case routing in the browser
...
- the same run lane now also uses `preferred_linked_case_action`, so it can open the latest run-backed case when it is still open or fall back to the canonical linked run queue when that latest case has already closed, from both list rows and detail
- the same canonical `GET /api/v1/eval-runs` list/detail contracts now also expose typed `preferred_primary_action`, so the first row-level handoff on `/admin/eval-runs` can be driven from one backend-owned open-case-versus-open-report-versus-open-run decision
