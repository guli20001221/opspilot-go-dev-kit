...
- use the `Quick views` buttons on that page when you want common operator slices such as `Queued`, `Needs approval`, `Failed`, `Running`, `Succeeded`, `Workflow required`, `Approval required`, `No approval`, `Report tasks`, or `Approved tools` without manually setting the same filters repeatedly
