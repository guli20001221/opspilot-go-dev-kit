...
- use the `Quick views` buttons on that page when you want common operator slices such as `Needs approval`, `Failed`, `Running`, `Succeeded`, `Report tasks`, or `Approved tools` without manually setting the same filters repeatedly
