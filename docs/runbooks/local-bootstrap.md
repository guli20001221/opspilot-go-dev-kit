...
- `GET /api/v1/tasks` now supports `tenant_id`, `status`, `task_type`, `reason`, `requires_approval`, `created_after`, `created_before`, `limit`, and `offset` filters for operator listing, with the two timestamp filters parsed as RFC3339 values, and returns `has_more` plus `next_offset` while keeping per-task `audit_events` only on `GET /api/v1/tasks/{task_id}`
