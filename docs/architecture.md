...
- those same canonical eval-dataset list/detail contracts now also expose `preferred_latest_activity_action`, so the dataset lane can hand operators to the latest durable run or report without hardcoded browser-side activity routing
...
- the same eval-run read model now also carries `preferred_linked_case_action`, so the run lane can surface either a direct latest-case handoff or the canonical linked run queue from one backend-owned decision in both list rows and detail
- the same canonical eval-run list/detail contracts now also carry typed `preferred_primary_action`, so `/admin/eval-runs` row-level main handoff is backend-owned and can prefer linked-case reuse, durable report review, or the run detail itself without browser-side routing heuristics
