...
- the same canonical eval-case list/detail contract now also carries backend-owned `preferred_follow_up_action`, so `/admin/evals` can render `Create case` versus reuse actions from one typed read-model field instead of recomputing that decision from counts and IDs in the browser
- the same canonical eval-case list/detail contract now also carries `linked_case_summary`, so `/admin/evals` can render total/open/latest follow-up pressure from one backend-owned block instead of leaning on only `latest_follow_up_case_id`
- the same canonical eval-case list/detail contract now also carries typed `preferred_primary_action`, so `/admin/evals` can reuse linked-case history before falling back to `Create case` instead of mixing follow-up and linked-case signals in page code
...
- the same canonical eval-dataset list now also carries `preferred_primary_action`, so `/admin/eval-datasets` rows can drive the main operator handoff from one backend-owned case-versus-queue-versus-report-versus-run decision without opening detail first
...
- those same recent-run rows now also carry linked run-backed case summary plus a typed `preferred_case_action`, so `/admin/eval-datasets` can hand off directly from recent activity into the latest run-backed case or run-backed queue without page-side routing heuristics
- those same recent-run rows now also carry a typed `preferred_follow_up_action`, so `/admin/eval-datasets` can hand operators into the canonical unresolved report-or-run queue for one recent execution without browser-side report-versus-run routing logic
- those same recent-run rows now also carry a typed `preferred_primary_action`, so `/admin/eval-datasets` can drive the main recent-activity operator button from one backend-owned decision instead of mixing run, report, queue, and linked-case heuristics in page code
- the same dataset lane now also surfaces latest-report open follow-up case pressure plus a canonical handoff into `/admin/cases`, so `baseline -> evidence -> work queue` stays connected without frontend-owned queue logic
- the same canonical dataset detail now also carries a `follow_up_case_summary` block, so `/admin/eval-datasets` can expose total/open/closed linked-case pressure and latest case status before an operator leaves the baseline lane
- the canonical case list contract now also supports `source_eval_dataset_id`, and eval-dataset list/detail reads now expose dataset-wide follow-up case summary plus a typed dataset queue action, so `/admin/eval-datasets` can hand operators into one dataset-scoped follow-up queue across all linked reports instead of only the latest-report queue
- the same canonical eval-dataset list rows now also expose `dataset_follow_up_case_summary`, so the dataset lane can show dataset-wide total/open/closed linked-case pressure without requiring a detail fetch first
...
- the same canonical eval-dataset list/detail contracts now also expose `preferred_run_backed_case_action`, so the dataset lane can delegate latest run-backed case-versus-queue routing back to the backend instead of inferring it from counts in page helpers
...
- the same canonical eval-dataset list/detail contracts now also expose `preferred_run_backed_case_action`, so dataset triage can hand off into the latest run-backed case or run-backed queue without page-side route heuristics
- the same canonical eval-dataset detail now also exposes `linked_case_summary` and `preferred_linked_case_action` on each `items[]` member, so dataset-member handoff into eval-case follow-up stays backend-owned instead of branching on IDs in `/admin/eval-datasets`
- those same dataset-detail `items[]` rows now also expose `preferred_follow_up_action`, so member-level create-versus-reuse case handoff stays backend-owned instead of requiring `/admin/eval-datasets` to infer it from linked-case counts
- those same dataset-detail `items[]` rows now also expose `preferred_primary_action`, so `/admin/eval-datasets` can drive the main member-level follow-up button from one backend-owned create-versus-reuse decision instead of mixing linked-case and follow-up heuristics in page code
...
- that same per-bad-case eval-report detail now also exposes a typed `preferred_follow_up_action`, so `/admin/eval-reports` consumes one backend-owned create-versus-reuse decision instead of recomputing it from follow-up counts in browser code
...
- `/admin/eval-reports` list rows now also consume the canonical `preferred_follow_up_action`, so row-level create-versus-reuse handoff stays backend-owned instead of becoming another browser-only decision layer
- the same `/admin/eval-reports` list now also hands a row straight into `/admin/eval-reports?bad_case_needs_follow_up=true&report_id=...`, so operators can jump from list triage into one report's unresolved bad-case slice without first opening detail
- the same canonical eval-report list/detail and compare-side contracts now also carry `preferred_bad_case_queue_action`, so unresolved bad-case queue handoff stays backend-owned instead of checking uncovered-count plus `report_id` in browser code
- the same canonical eval-report list/detail contract now also carries typed `preferred_compare_follow_up_action`, so `/admin/eval-reports` can hand compare-origin follow-up straight into the canonical compare queue without checking compare follow-up counts in browser code
- the same canonical eval-report detail now also carries `linked_case_summary`, so `/admin/eval-reports` renders linked-case counts and latest ownership from one backend-owned detail read instead of issuing a second case-list fetch
- the same canonical eval-report list now also carries `linked_case_summary`, so `/admin/eval-reports` rows can surface latest linked-case owner and linked-open pressure without requiring a detail round-trip first
- the same canonical eval-report list/detail contract now also carries `preferred_linked_case_action`, so linked-case handoff stays backend-owned instead of using browser-side `latest_follow_up_case_id` heuristics
- the same canonical eval-report list/detail contract now also carries typed `preferred_primary_action`, so `/admin/eval-reports` can reuse linked-case history before falling back to `Create case` instead of mixing follow-up and linked-case signals in browser code
- the same canonical eval-report detail now also carries typed `preferred_primary_action` on each `bad_cases[]` row, so `/admin/eval-reports` can drive bad-case create-versus-reuse handoff from one backend-owned decision instead of mixing follow-up and linked-case signals in page code
- the same canonical eval-report detail now also carries `preferred_linked_case_action` on each bad case, so bad-case linked-case handoff reuses one backend-owned decision instead of branching on `latest_follow_up_case_id` in the browser
...
- that same eval-run detail now also carries backend-owned `preferred_follow_up_action` plus `latest_follow_up_case_id` on both `items` and `item_results`, so `/admin/eval-runs` can drive failed-item follow-up directly from the canonical contract instead of re-deriving create-versus-reuse behavior in the browser
- those same eval-run `items` and `item_results` now also carry `linked_case_summary`, so `/admin/eval-runs` can render per-result total/open/latest follow-up pressure from backend-owned case lineage without a second case lookup
...
- those same eval-run `items` and `item_results` now also carry typed `preferred_primary_action`, so `/admin/eval-runs` can reuse linked-case history before it falls back to `Create case from result` instead of mixing follow-up and linked-case signals in page code
- the canonical eval-run list now also carries `item_without_open_follow_up_count` plus `needs_follow_up`, so `/admin/eval-runs` can offer a backend-owned unresolved follow-up queue without issuing per-row case-summary inference in the browser
- terminal eval-run reads now also carry canonical `report_id` plus `report_status` when the durable eval-report artifact exists, so `/admin/eval-runs` can hand off directly into report review without recomputing run-to-report lineage in the page
- the same canonical eval-run list/detail contracts now also expose `linked_case_summary`, so run-level triage can show the latest linked follow-up case status and assignment without opening `/admin/cases`
...
- that same eval-run linked-case summary is now computed from durable `source_eval_run_id` case lineage instead of being reconstructed from per-item eval-case follow-up, so run-backed case queues remain visible even before any per-item case exists
- the same eval-run lineage now also has an additive backfill migration for legacy cases created before `source_eval_run_id` existed, but it only fills rows that match the old canonical eval-run handoff summary pattern and run-item membership unambiguously
...
- the same canonical eval-report compare contract now also carries backend-owned `preferred_compare_follow_up_action` per side, so `/admin/eval-report-compare` can drive its primary left/right follow-up buttons from typed contract state instead of recomputing that decision from compare-follow-up counts in the browser
- that same canonical eval-report compare contract now also carries per-side `linked_case_summary`, so compare-time triage can see total/open linked follow-up pressure plus latest ownership without a second case-read path
- that same canonical eval-report compare contract now also carries per-side `preferred_linked_case_action`, so compare-time linked-case handoff opens the latest linked case or the canonical linked-case queue from one backend-owned decision instead of checking `latest_follow_up_case_id` in browser code
- that same canonical eval-report compare contract now also carries per-side `preferred_primary_action`, so the compare lane can reuse linked-case history before falling back to compare-queue or create decisions instead of mixing those signals in page code
