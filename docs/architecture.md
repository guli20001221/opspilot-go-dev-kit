...
- the same eval-run read model now also carries `preferred_linked_case_action`, so the run lane can surface either a direct latest-case handoff or the canonical linked run queue from one backend-owned decision
