...
- the same `/admin/cases` page now also supports `Return to queue`, backed by canonical `POST /api/v1/cases/{case_id}/unassign`, so claimed work can move back into the shared unassigned lane without a detail-only workaround
