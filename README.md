...
- the same eval-run contracts now also expose a typed `preferred_linked_case_action`, so `/admin/eval-runs` can open the latest run-backed case when it is still open or fall back to the canonical linked run queue when the latest case has already closed
