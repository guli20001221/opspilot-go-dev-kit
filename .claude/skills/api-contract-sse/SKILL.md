...
18. When operator UIs need to decide between `create` and `open existing` follow-up actions for a durable eval artifact, prefer a typed backend-owned read-model field such as `preferred_follow_up_action` over re-deriving that decision from counts and IDs in the browser.
19. The first eval dataset contract should be a canonical `POST /api/v1/eval-datasets` plus `GET /api/v1/eval-datasets/{id}` backed by durable membership rows, not an admin-only batch action.
20. Once durable dataset drafts exist, expose a tenant-scoped `GET /api/v1/eval-datasets` with stable `updated_at DESC, id DESC` ordering and lightweight rows instead of returning full memberships from the list contract.
21. The first dataset-curation mutation should be an explicit `POST /api/v1/eval-datasets/{dataset_id}/items` append contract that is idempotent for the same eval case and returns the updated dataset detail.
22. Once dataset drafts can be curated incrementally, add an explicit `POST /api/v1/eval-datasets/{dataset_id}/publish` contract that returns the updated dataset detail and surfaces repeated publish attempts as a typed 409-style invalid-state error.
23. Once datasets can be published, add a canonical `POST /api/v1/eval-runs`, tenant-scoped `GET /api/v1/eval-runs`, and `GET /api/v1/eval-runs/{run_id}` contract that snapshots published dataset metadata into a durable queued run before execution is wired.
24. The first eval-run execution slice should keep `started_at`, `finished_at`, `status`, and `error_reason` on that same canonical run detail contract instead of inventing a second execution-status surface.
25. Recovery on eval runs should reuse that same canonical record; prefer `POST /api/v1/eval-runs/{run_id}/retry` with a typed 409 invalid-state error over creating a separate rerun resource or admin-only mutation path.
26. When retry on a durable eval run clears top-level failure fields, extend only the single-run detail contract with append-only lifecycle events; keep the list contract lightweight.
27. When per-run judging has not landed yet, prefer adding immutable `items` only on the single-run detail contract, copied from published dataset membership at kickoff time, and keep create/list/retry responses lightweight snapshots.
...
32. Once `source_eval_run_id` is part of the canonical case contract, prefer `POST /api/v1/cases` to reuse the newest open `tenant_id + source_eval_run_id` case instead of creating duplicate run-backed follow-up rows from repeated operator clicks.
33. Once canonical eval-dataset reads already expose both latest-report and dataset-wide case queue actions, add a typed preferred case handoff field on that same contract instead of leaving the browser to compose dataset-versus-report queue priority on its own.
34. Once canonical eval-dataset reads already know the latest durable run ID, expose a lightweight run-backed case summary there instead of forcing operators to query `/api/v1/eval-runs` or `/api/v1/cases` just to see whether the latest run already has claimed follow-up work.
...
- once canonical eval-report compare reads need one main side-level follow-up decision, prefer exposing `preferred_primary_action` on those same compare items instead of leaving `/admin/eval-report-compare` to mix linked-case and compare-follow-up summaries in browser code
...
- once canonical eval-report list/detail reads need one main report-level follow-up decision, prefer exposing `preferred_primary_action` on those same contracts instead of leaving admin pages to mix linked-case and follow-up summaries in browser code
...
- once canonical eval-case reads already expose `linked_case_summary`, add a typed `preferred_linked_case_action` on that same contract instead of leaving `/admin/evals` to branch on `latest_follow_up_case_id`
- once canonical eval-case reads need one main eval-case follow-up decision, prefer exposing `preferred_primary_action` on that same contract instead of leaving `/admin/evals` to mix follow-up and linked-case summaries in browser code
- once canonical eval-run detail rows already expose `linked_case_summary`, add a typed `preferred_linked_case_action` on those same `items` and `item_results` instead of leaving `/admin/eval-runs` to branch on `latest_follow_up_case_id`
- once canonical eval-report reads already expose unresolved bad-case pressure, add a typed `preferred_bad_case_queue_action` on those same list/detail and compare-side contracts instead of leaving admin pages to compose unresolved-report queue URLs from `report_id`
- once canonical eval-dataset detail already exposes durable `items[]`, add `linked_case_summary` and a typed `preferred_linked_case_action` on those same member rows instead of leaving `/admin/eval-datasets` to infer member-level follow-up routing from raw eval-case IDs
- once canonical eval-dataset detail already exposes durable `items[]`, add a typed `preferred_follow_up_action` on those same member rows instead of leaving `/admin/eval-datasets` to infer member-level create-versus-reuse case handoff from linked-case counts
