...
- once canonical eval-report detail exposes a typed `preferred_provenance_action` on each bad case, prefer wiring the main bad-case provenance handoff from that backend-owned field instead of branching on `source_case_id`, `trace_id`, or `version_id` in browser code
